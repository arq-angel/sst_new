...
class DepartmentController
{
    ...
    public static function add()
    {
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            if (CsrfProtection::validateToken($token)) {
                ...
                if (!$errors) {
                    $result = CreateRecord::loadCreateRecord('department', $data);

                    if ($result) {
                        header('Location: ' . Site::ROOT_URL . '/app/department');
                        exit;
                    }
                } else {
                    self::renderDepartment([
                        'data' => $data,
                        'errors' => $errors,
                    ]);
                }
    ...
    public static function renderDepartment(array $params = [])
    {
